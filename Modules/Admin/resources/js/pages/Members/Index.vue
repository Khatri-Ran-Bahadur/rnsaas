...
const openPasswordModal = (member: MemberItem) => {
...
const submitChangePassword = () => {
    ...
    passwordForm.patch(`/admin/members/${activeMember.value.id}/password`, {
        onSuccess: () => {
...
const activateInvitation = (member: MemberItem) => {
    router.patch(`/admin/members/${member.id}/activate`, {}, {
        preserveScroll: true,
    });
};
    ...
    >
                                        ...
                                        <Dropdown align="right" width="w-52">
                                            ...
                                            <template #default="{ close }">
                                                <div class="py-1 text-xs">
                                                    ...
                                                    <!-- Accept & Activate Invitation (If Invited) -->
                                                    <button
                                                        ...
                                                        @click="activateInvitation(member); close()"
                                                    >
                                                    ...
                                                    <!-- Change Password (If Active) -->
                                                    <button
                                                        v-if="member.status === 'active'"
                                                        type="button"
                                                        class="flex w-full items-center gap-2.5 px-3 py-2 text-zinc-700 hover:bg-zinc-100 dark:text-zinc-200 dark:hover:bg-zinc-800 transition-colors rounded-md text-left cursor-pointer"
                                                        @click="openPasswordModal(member); close()"
                                                    >
        ...
        >
            <form @submit.prevent="submitChangePassword" class="space-y-4">
                <p class="text-xs text-zinc-600 dark:text-zinc-300">
                    Change password for <strong v-if="activeMember">{{ activeMember.user.name }}</strong> ({{ activeMember?.user.email }}).
                </p>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input
                            ...
                            required
                            minlength="6"
                            class="w-full rounded-xl border border-zinc-200 bg-zinc-50 pl-3.5 pr-10 py-2 text-xs text-zinc-900 placeholder-zinc-400 focus:border-violet-500 focus:bg-white dark:focus:bg-zinc-900 focus:outline-hidden dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder-zinc-500"
                        />
                        <button
                            type="button"
                    ...
                    <p v-if="passwordForm.errors.password" class="mt-1 text-xs text-rose-500">
                        {{ passwordForm.errors.password }}
                    </p>
                </div>

                <div class="flex justify-end gap-3 pt-4 border-t border-zinc-100 dark:border-zinc-800">
                    <Button variant="outline" size="sm" type="button" @click="isPasswordModalOpen = false">
                        Cancel
                    </Button>
                    <Button variant="primary" size="sm" type="submit" :disabled="passwordForm.processing">
                        Update Password
                    </Button>
                </div>
            </form>
